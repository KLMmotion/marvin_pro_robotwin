...
class VirtualCameraLink:
    """Lightweight wrist camera pose provider that does not participate in physics."""

    def __init__(self, parent_link, local_pose: sapien.Pose, name: str):
        self.parent_link = parent_link
        self.local_pose = local_pose
        self.name = name

    def get_pose(self):
        if hasattr(self.parent_link, "get_entity_pose"):
            parent_pose = self.parent_link.get_entity_pose()
        else:
            parent_pose = self.parent_link.get_pose()
        return parent_pose * self.local_pose
...
class Robot:
    ...
    def _init_robot_(self, scene, need_topp=False, **kwargs):
        ...
        self.left_js = None
        self.right_js = None
        self.left_planner = getattr(self, "left_planner", None)
        self.right_planner = getattr(self, "right_planner", None)
        self.communication_flag = getattr(self, "communication_flag", False)

        left_embodiment_args = kwargs["left_embodiment_config"]
        right_embodiment_args = kwargs["right_embodiment_config"]
        self.left_embodiment_args = left_embodiment_args
        self.right_embodiment_args = right_embodiment_args
        ...
        self.disable_robot_self_collision = (
            kwargs.get("disable_robot_self_collision", False)
            or left_embodiment_args.get("disable_self_collision", False)
            or right_embodiment_args.get("disable_self_collision", False)
        )
        self.disable_collision_pairs = self._collect_disable_collision_pairs(
            kwargs.get("disable_collision_pairs", []),
            left_embodiment_args.get("disable_collision_pairs", []),
            right_embodiment_args.get("disable_collision_pairs", []),
        )
        self.plan_from_drive_target = (
            kwargs.get("plan_from_drive_target", True)
            and left_embodiment_args.get("plan_from_drive_target", True)
            and right_embodiment_args.get("plan_from_drive_target", True)
        )
        ...
        if self.disable_robot_self_collision:
            self._disable_self_collision(self.left_entity)
            if self.right_entity is not self.left_entity:
                self._disable_self_collision(self.right_entity)
        if self.disable_collision_pairs:
            self._disable_collision_pairs(self.left_entity, self.disable_collision_pairs)
            if self.right_entity is not self.left_entity:
                self._disable_collision_pairs(self.right_entity, self.disable_collision_pairs)

    def _collect_disable_collision_pairs(self, *pair_groups):
        pairs = []
        seen = set()
        for group in pair_groups:
            for pair in group or []:
                if len(pair) != 2:
                    raise ValueError(f"disable_collision_pairs entry must contain two link names, got: {pair}")
                link1, link2 = str(pair[0]), str(pair[1])
    ...
    def _disable_self_collision(self, entity, ignore_bit=1, ignore_id=1):
        for link in entity.get_links():
            for shape in link.get_collision_shapes():
                groups = list(shape.get_collision_groups())
                groups[2] = groups[2] | ignore_bit
                groups[3] = (groups[3] & 0xFFFF0000) | ignore_id
                shape.set_collision_groups(groups)

    def _disable_collision_pairs(self, entity, pairs, ignore_id=1):
        if len(pairs) > 30:
            raise ValueError(
                "disable_collision_pairs supports at most 30 pairs with the current "
                f"SAPIEN collision-group encoding, got {len(pairs)}"
            )

        link_map = {link.get_name(): link for link in entity.get_links()}
        for pair_id, (link1_name, link2_name) in enumerate(pairs):
            link1 = link_map.get(link1_name)
            link2 = link_map.get(link2_name)
            if link1 is None or link2 is None:
                missing = [name for name, link in ((link1_name, link1), (link2_name, link2)) if link is None]
                print(f"[Robot] skip disable_collision_pair {link1_name} <-> {link2_name}, missing links: {missing}")
                continue

            ignore_bit = 1 << pair_id
    ...
    def _get_planning_qpos(self, entity, arm_tag, last_qpos=None):
        if last_qpos is not None:
            return deepcopy(last_qpos)
        qpos = entity.get_qpos()
        if not self.plan_from_drive_target:
            return qpos
    ...
    def _build_virtual_camera_link(self, entity, embodiment_args, side: str):
        wrist_camera_cfg = embodiment_args.get("wrist_camera_pose", {})
        camera_cfg = wrist_camera_cfg.get(side)
        if camera_cfg is None:
            return None

        parent_name = camera_cfg.get("parent", f"{side}_tool")
        parent_link = entity.find_link_by_name(parent_name)
        if parent_link is None:
            print(f"No {side} virtual camera parent link: {parent_name}")
            return None

        xyz = camera_cfg.get("xyz", [0, 0, 0])
        rpy = camera_cfg.get("rpy", [0, 0, 0])
        quat = t3d.euler.euler2quat(float(rpy[0]), float(rpy[1]), float(rpy[2]))
        return VirtualCameraLink(parent_link, sapien.Pose(xyz, quat), f"{side}_camera")
    ...
    def init_joints(self):
        ...
        # camera link id. Prefer virtual wrist cameras when configured so camera
        # extrinsics do not perturb the robot articulation dynamics.
        use_left_virtual_camera = self.left_embodiment_args.get("use_virtual_wrist_camera", False)
        use_right_virtual_camera = self.right_embodiment_args.get("use_virtual_wrist_camera", False)

        self.left_camera = (self._build_virtual_camera_link(self.left_entity, self.left_embodiment_args, "left")
                            if use_left_virtual_camera else None)
        ...
        self.right_camera = (self._build_virtual_camera_link(self.right_entity, self.right_embodiment_args, "right")
                             if use_right_virtual_camera else None)
    ...
    def set_planner(self, scene=None):
        ...
        # When the embodiment provides separate left/right cuRobo configs, prefer them
        # even if both arms come from the same embodiment directory.
        left_split_yml_path = abs_left_curobo_yml_path.replace("curobo.yml", "curobo_left.yml")
        right_split_yml_path = abs_right_curobo_yml_path.replace("curobo.yml", "curobo_right.yml")
        if os.path.exists(left_split_yml_path) and os.path.exists(right_split_yml_path):
            abs_left_curobo_yml_path = left_split_yml_path
            abs_right_curobo_yml_path = right_split_yml_path
    ...
    def left_plan_multi_path(
        ...
        now_qpos = self._get_planning_qpos(self.left_entity, "left", last_qpos)
    ...
    def right_plan_multi_path(
        ...
        now_qpos = self._get_planning_qpos(self.right_entity, "right", last_qpos)
        target_lst_copy = deepcopy(target_lst)
        for i in range(len(target_lst_copy)):
