...
try:
    ...
    class CuroboPlanner:
        ...
        def __init__(
            ...
            planner_cfg = yml_data.get("planner", {})
            self.frame_bias = planner_cfg["frame_bias"]
            self.debug_override_pose = planner_cfg.get("debug_override_pose", None)
            if self.debug_override_pose is not None and len(self.debug_override_pose) != 7:
                raise ValueError("planner.debug_override_pose must be [x, y, z, qw, qx, qy, qz] in base frame.")
            self._printed_debug_override = False
            ...
            motion_gen_config = MotionGenConfig.load_from_robot_config(
                ...
                num_trajopt_seeds=1,
                # Temporary calibration setting:
                # Tianji's retract/home state is flagged as self-colliding by cuRobo's
                # collision spheres, which blocks all debug moves before planning starts.
        ...
        def plan_path(
            ...
            target_pose_p = [0.28, 0.14, 0.93]
            target_pose_q = [1., 0., 0., 0.]

            # target_pose_p[0] += self.frame_bias[0]
            # target_pose_p[1] += self.frame_bias[1]
            # target_pose_p[2] += self.frame_bias[2]
            # target_pose_p = np.array([0.35, 0.08, 1.04])
            ...
            if result.success.item() == False:
                print(
                    f"[curobo plan_path] {arms_tag} failed for hardcoded ee pose "
                    f"p={target_pose_p}, q={target_pose_q}"
                )
