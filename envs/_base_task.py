...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        self.render_sleep = kwags.get("render_sleep", 0)
    ...
    def load_robot(self, **kwags):
        ...
        """
        robot_kwargs = {k: v for k, v in kwags.items() if k != "need_topp"}
    ...
    def set_gripper(self, set_tag="together", left_pos=None, right_pos=None):
        ...
        def direct_gripper_plan(now_val, target_val, step_size=0.02):
            target_val = float(np.clip(target_val, 0, 1))
            now_val = float(np.clip(now_val, 0, 1))
            num_step = max(1, int(np.ceil(abs(target_val - now_val) / step_size)))
        ...
        if set_tag == "left" or set_tag == "together":
            if self.need_plan:
                ...
                left_result = direct_gripper_plan(self.robot.get_left_gripper_val(), left_pos)
        ...
        if set_tag == "right" or set_tag == "together":
            if self.need_plan:
                ...
                right_result = direct_gripper_plan(self.robot.get_right_gripper_val(), right_pos)
    ...
    def left_move_to_pose(
        ...
        if left_result["status"] != "Success":
            print(f"[left_move_to_pose] planning failed, requested pose={pose}, result={left_result}")
            self.plan_success = False
            return
    ...
    def right_move_to_pose(
        ...
        if right_result["status"] != "Success":
            print(f"[right_move_to_pose] planning failed, requested pose={pose}, result={right_result}")
            self.plan_success = False
            return
    ...
    def choose_best_pose(self, res_pose, center_pose, arm_tag: ArmTag = None):
        ...
        if now_pose is None:
            print("[choose_best_pose] no valid candidate pose")
            print(f"  arm_tag: {arm_tag}")
            print(f"  requested_pose: {res_pose}")
            print(f"  center_pose: {center_pose}")
            print(f"  candidate_count: {pose_num}")
            print(f"  planner_status: {traj_lst.get('status') if isinstance(traj_lst, dict) else traj_lst}")
        return now_pose
    ...
    def choose_grasp_pose(
        ...
        def get_plan_func(arm_tag):
            if arm_tag == "left":
                return self.robot.left_plan_path
            return self.robot.right_plan_path

        def get_plan_multi_func(arm_tag):
            if arm_tag == "left":
                return self.robot.left_plan_multi_path
            return self.robot.right_plan_multi_path

        # 由 hzh 补充：cuRobo 的单臂规划结果只包含该侧 7 个关节时，需要还原成完整 qpos，
        # 才能作为下一段 grasp 约束规划的起点，避免从错误的当前状态继续规划。
        def full_qpos_after_arm_path(arm_tag, arm_qpos):
            ...
            if len(arm_qpos) == len(qpos):
                return arm_qpos
            for value, joint in zip(arm_qpos, arm_joints):
                qpos[active_joints.index(joint)] = value
            return qpos

        # 由 hzh 补充：保留原始 contact point 到 pre-grasp pose 的计算，
        # 后面会基于这个原始 pose 枚举所有旋转候选，而不是只取默认最短路径候选。
        def get_raw_pre_grasp_pose(contact_id):
            ...
            global_grasp_pose_q = t3d.quaternions.mat2quat(global_contact_pose_matrix_q)
            return list(global_grasp_pose_p) + list(global_grasp_pose_q), actor.get_contact_point(contact_id, "list")

        # 由 hzh 补充：抓取候选不仅要能到达 pre-grasp，还必须能从 pre-grasp
        # 沿约束方向继续规划到真正 grasp pose，否则夹爪会停在 pre-grasp 或把物体顶开。
        def check_pose(pre_pose, pose, arm_tag, pre_path=None):
            plan_func = get_plan_func(arm_tag)
            if pre_path is None:
                pre_path = plan_func(pre_pose)
                if pre_path["status"] != "Success":
                    return False
                pre_arm_qpos = pre_path["position"][-1]
            else:
                pre_arm_qpos = pre_path
            pre_qpos = full_qpos_after_arm_path(arm_tag, pre_arm_qpos)
            grasp_path = plan_func(
                pose,
                constraint_pose=[1, 1, 1, 0, 0, 0],
                last_qpos=pre_qpos,
            )
            return grasp_path["status"] == "Success"
        ...
        # 由 hzh 补充：遍历每个 contact point 的全部旋转候选，并且过滤掉
        # pre-grasp 成功但 grasp 失败的“假可行”候选，再按原有姿态分数选择最优抓取。
        for i, _ in contact_point_id:
            raw_pre_pose, center_pose = get_raw_pre_grasp_pose(i)
            if raw_pre_pose is None:
                continue
            target_lst = self.robot.create_target_pose_list(raw_pre_pose, center_pose, arm_tag)
            traj_lst = get_plan_multi_func(arm_tag)(target_lst)
            statuses = traj_lst.get("status") if isinstance(traj_lst, dict) else traj_lst
            if "position" not in traj_lst:
                continue

            for candidate_idx, pre_pose in enumerate(target_lst):
                if statuses[candidate_idx] != "Success":
                    continue
                pose = get_grasp_pose(pre_pose, pre_dis - target_dis)
                if not check_pose(pre_pose, pose, arm_tag, traj_lst["position"][candidate_idx][-1]):
                    continue
    ...
    def grasp_actor(
        ...
        if pre_grasp_pose is None or grasp_pose is None:
            raise RuntimeError(
                ...
                f"pre_grasp_dis={pre_grasp_dis}, grasp_dis={grasp_dis}, contact_point_id={contact_point_id}"
            )
        ...
        **args,
        ...
        if self.need_plan:
            ...
            if place_pre_pose is None or place_pose is None:
                raise RuntimeError(
                    f"No valid place pose for actor={actor.get_name()}, arm={arm_tag}, "
                    f"target_pose={target_pose}, functional_point_id={functional_point_id}, "
                    f"pre_dis={pre_dis}, dis={dis}"
                )
        else:
